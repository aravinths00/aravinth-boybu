 ...
 *
 * Copyright (c) 2021 Aravinth.
 *
 * For the full copyright and license information, please view the LICENSE.md
 * file that was distributed with this source code.
 */

namespace Aravinth\Boybu;
...
return [
    ...
    (new Extend\Frontend('forum'))
        ->route('/private', 'boybuPrivate', Content\PrivateDiscussionsPage::class)
    ...
    (new Extend\ApiSerializer(Serializer\UserSerializer::class))
        ->attribute('blocksPd', function ($serializer, $user) {
            return (bool) $user->blocks_boybu_pd;
        })
    ...
    (new Extend\View())
        ->namespace('aravinth-boybu', __DIR__.'/resources/views'),
    ...
    (new Extend\ServiceProvider())
        ->register(Provider\BoybuProvider::class),
    ...
    (new Extend\Settings())
        ->serializeToForum('boybu.icon-badge', 'aravinth-boybu.icon-badge', function ($value) {
            return empty($value) ? 'fas fa-map' : $value;
        })
        ->serializeToForum('boybu.icon-postAction', 'aravinth-boybu.icon-postAction', function ($value) {
